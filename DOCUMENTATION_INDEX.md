...
**Implemented and merged.** The approved Milestone 15 documents remain specification authority and the merged implementation provides strict runtime contracts, deterministic genesis, durable registration, permanent identity ownership, dual-time fresh-evaluator replay, marker-bounded recovery/integrity, derived-state rebuilding, and the cooperative local adapter. Historical reconstruction uses immutable original time; current admissibility uses explicit replay time. Atomic installation of the byte-identical fixed current-marker copy is the sole visibility boundary; the marker-embedded head and `readHead()` are byte-identical, while separate `HEAD` projections and indexes remain derived. Registration/replay failure results use schema-owned closed status-specific reason enums, and all 72 catalog scenarios use harness-observed production boundaries plus exact assertion contracts without hard-coded invocation evidence or generic fallback behavior. Integrity, recovery, rebuild, and application result envelopes remain ephemeral and non-fingerprinted. A strict versioned preflight independently inspects the authorized Git coordinates, and the standalone `pnpm verify:m15-predecessor` command proves the 42-file/1,038-test predecessor baseline without nesting Vitest inside the ordinary scenario lifecycle. ADR-0019 is Accepted. Its acceptance covers only the durable non-executing ledger; credential resolution, provider transport, real-provider integration, and live-execution authority are not authorized.
...
**Accepted and merged — documentation-only; no implementation authorized by this milestone.** Milestone 16 selects the OpenAI provider family and one future foreground non-streaming text-only operation, `founder-decision-memo`, from one verified governed Context Package. It defines a closed Responses API request envelope, authority and credential ownership, transport and retention boundaries, a 25-row threat model, fail-closed behavior, incident and kill-switch policy, and a separately gated implementation sequence. The documentation candidate was independently reviewed and merged through pull request #16; ADR-0020 is Accepted. The package adds no credential, resolver, provider SDK, network path, live request, deployment, or execution authority.
...
**Accepted and merged — non-production and process-local.** Milestone 17 adds strict provider-neutral Service Identity, human approval, Authorization Request, Decision, claim, result, deterministic fingerprint, exact Service Identity evidence/workload/proof and fixed processing-tier binding, exact issuance, atomic permanent claim, monotonic revocation, inspection, verification, and disabled-harness behavior. The implementation was independently reviewed and merged through pull request #17; ADR-0021 is Accepted. It adds no identity provider, credential resolution, durable or distributed authorization storage, provider mapping, transport, network, deployment, or live execution.
...
- [Repository Audit v2.0 — current](./docs/reviews/REPOSITORY_AUDIT_v2.0.md)
- [Repository Audit v1.0 — historical Milestone 03 baseline](./docs/reviews/REPOSITORY_AUDIT_v1.0.md)
