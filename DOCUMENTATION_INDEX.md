...
**Implementation candidate — independent review pending.** The approved Milestone 15 documents remain specification authority while the local candidate implements strict runtime contracts, deterministic genesis, durable registration, permanent identity ownership, dual-time fresh-evaluator replay, marker-bounded recovery/integrity, derived-state rebuilding, and the cooperative local adapter. Historical reconstruction uses immutable original time; current admissibility uses explicit replay time. Atomic installation of the byte-identical fixed current-marker copy is the sole visibility boundary; the marker-embedded head and `readHead()` are byte-identical, while separate `HEAD` projections and indexes remain derived. Registration/replay failure results use schema-owned closed status-specific reason enums, and all 72 catalog scenarios use harness-observed production boundaries plus exact assertion contracts without hard-coded invocation evidence or generic fallback behavior. Integrity, recovery, rebuild, and application result envelopes remain ephemeral and non-fingerprinted. A strict versioned preflight independently inspects the authorized Git coordinates, and the standalone `pnpm verify:m15-predecessor` command proves the 42-file/1,038-test predecessor baseline without nesting Vitest inside the ordinary scenario lifecycle. ADR-0019 remains Proposed; publication, credential resolution, provider transport, real-provider integration, and live-execution authority are not authorized.
