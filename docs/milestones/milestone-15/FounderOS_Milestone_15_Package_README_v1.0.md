...
Each event retains one immutable archived commit-marker value. Atomic installation of byte-identical bytes at the fixed current-marker location is the sole authoritative visibility boundary; the archive preserves history but cannot activate visibility. The embedded resulting head commits the complete event, while separate `HEAD` projections and indexes are derived and rebuildable. The transaction contract contains the sole normative commitment-domain table, the local adapter contains the sole 19-row fault-point matrix, and the acceptance criteria contain clause-level source ownership plus the normative requirement-to-scenario traceability matrix.
